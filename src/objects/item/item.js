...
export class Item extends Object {
    ...
    respawn(player) {
        ...
        if (this.timetokill <= 0) {
            ...
            camera.updateMatrixWorld();
            if (this.reset < 0) {
                this.x = camera.position.x + this.getRandomInt(50, 200) * (this.getRandomInt(-2, 2) > 0 ? 1 : -1);
                this.y = camera.position.y - 2;
                this.z = camera.position.z + this.getRandomInt(50, 200) * (this.getRandomInt(-2, 2) > 0 ? 1 : -1);
